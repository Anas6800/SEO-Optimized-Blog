...
export default function PostCard({ post }: { post: NormalizedPost }) {
	...
	return (
		<article className="card-surface overflow-hidden flex flex-col hover:shadow-lg hover:shadow-indigo-500/10 transition-all duration-300 h-full group">
			...
			<div className="p-5 flex flex-col gap-3 flex-1">
				...
				<p className="text-sm text-gray-300 line-clamp-3 min-h-[4.5rem] flex-grow leading-relaxed">
							...
							{post.categories.slice(0, 3).map((c) => (
								<li key={c.id}>
									<Link 
										...
										className="inline-flex items-center px-2.5 py-1 rounded-full text-xs bg-indigo-500/20 text-indigo-200 border border-indigo-500/30 hover:bg-indigo-500/30 hover:border-indigo-400/50 transition-all duration-200"
									>
										{c.name}
									</Link>
								</li>
